...
V = MTeasyPy.V(np.array([sqrt(500./2),0.00000000001])) # calculate potential from some initial conditions
dV = np.zeros(nF) # calculat1e derivatives of potential
MTeasyPy.dV(np.array([sqrt(500./2),0.00000000001]),dV) # calculate derivatives of potential (changes dV to derivatives)

initial = np.array([sqrt(500./2),0.000000000001,-dV[0]/sqrt(3.*V),0.0]) # set initial conditions to be in slow roll
Nstart = 0.0
Nend = 50.0
t=np.linspace(Nstart, Nend, 1000)
# solve a fiducial background run
back2=MTeasyPy.backEvolve(t, initial)
...
sig = MTeasyPy.sigEvolve(t, k, backExitMinus, 1)
...
t=np.linspace(Nstart,Nend, 1000)
alp=MTeasyPy.alphaEvolve(t,k1,k2,k3, backExitMinus,1)
