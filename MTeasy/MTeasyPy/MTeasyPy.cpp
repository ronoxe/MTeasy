...
// Package recompiled: Thu Jul 28 14:02:56 2016
...
static PyObject* MT_sigEvolve(PyObject* self,  PyObject *args)
{
    ...
    double *CinitialCs, *tc, k, *Cparams;// swap k with kin
    int full;
	//double k=1.0;
	...
	double rtol=pow(10,-8); double atol=pow(10,-10) ; 
    ...
    for(int ii=0; ii<nt; ii++ ){
        ...
        cout <<" " << (1.0*ii)/(1.0*nt) *100.0 << "% Complete"<< "\r" << std::flush;
    }
...
void initMTeasyPyPS(void)    {        Py_InitModule3("MTeasyPyPS", MTeasyPyPS_funcs,                       "Extension module for inflationary statistics");        import_array();   }//initFunc
