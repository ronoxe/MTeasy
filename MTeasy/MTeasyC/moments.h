...
private:
    ...
    alpha(int nFi, double k1, double k2, double k3, double N0, vector<double> f,vector<double> p)
    {
        ...
        fpf = pffCalc(f,p, k1, k2, k3, N0);
        ffp = pffCalc(f,p, k1, k2, k3, N0);
      
        ppf = ppfCalc(f,p, k1, k2, k3, N0);
        pfp = ppfCalc(f,p, k1, k2, k3, N0);
        fpp = ppfCalc(f,p, k1, k2, k3, N0);
        ...
        for(int i=0;i<nF;i++){for(int j=0;j<nF;j++){for(int k=0;k<nF;k++){
          alp[i + j*2*nF + k*2*nF*2*nF] = fff[i + j*nF + k*nF*nF] ;
            alp[nF+i + j*2*nF + k*2*nF*2*nF] = pff[i + j*nF + k*nF*nF]/a  *a;
            alp[i + (nF+j)*2*nF + k*2*nF*2*nF] = fpf[j + i*nF + k*nF*nF]/a *a;
            alp[i + j*2*nF + (nF+k)*2*nF*2*nF ] = ffp[k + i*nF + j*nF*nF ]/a *a;
            alp[i + (nF+j)*2*nF + (nF+k)*2*nF*2*nF] = fpp[j + k*nF + i*nF*nF]/a/a *a*a;
            alp[(nF+i) + (nF+j)*2*nF + (k)*2*nF*2*nF] = ppf[i + j*nF + k*nF*nF ]/a/a *a*a;
            alp[(nF+i) + (j)*2*nF + (nF+k)*2*nF*2*nF] = pfp[i + k*nF + j*nF*nF ]/a/a *a*a;
            alp[(nF+i) + (nF+j)*2*nF + (nF+k)*2*nF*2*nF ] = ppp[i + j*nF + k*nF*nF ]/a/a/a *a*a*a;
        }}}
    }
